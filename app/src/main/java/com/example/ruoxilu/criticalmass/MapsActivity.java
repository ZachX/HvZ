...
//            });
    ...
    private void doMapQuery() {
        ...
        ParseQuery<MassUser> userQuery = MassUser.getQuery();
        // 4
        mapQuery.whereWithinKilometers("location", myPoint, SEARCH_DISTANCE);
        userQuery.whereWithinKilometers("location", myPoint, SEARCH_DISTANCE);
        // 5
...
//        });
        ...
        userQuery.findInBackground(new FindCallback<MassUser>() {
            @Override
            public void done(List<MassUser> objects, ParseException e) {
                ...
                for (MassUser mUser : objects) {
                    ...
                    // 5
                    MarkerOptions markerOpts = createMarkerOpt(mUser);

                    // new MarkerOptions().position(new LatLng(mUser.getLocation().getLatitude(), mUser.getLocation().getLongitude()));
                    // 6
                    // Set up an out-of-range marker
                    // Check for an existing out of range marker
                    if (oldMarker != null) {
    ...
    protected MarkerOptions createMarkerOpt(MassUser mUser){

        String status = mUser.getStatus();
        Log.d(APPTAG, "User status is " + status);
        if(status.equals("human")){
            Log.d(APPTAG, "HUMAN");
            MarkerOptions markerOpt = new MarkerOptions().position(
                    new LatLng(mUser.getLocation().getLatitude(), mUser
                            .getLocation().getLongitude())).icon(BitmapDescriptorFactory.fromResource(R.drawable.pinicon));
            return markerOpt;
        } else {
            Log.d(APPTAG, "ZOMBIE");
            MarkerOptions markerOpt = new MarkerOptions().position(
